...
WITH nav_change AS (
			SELECT 
				...
				p.NAV_USD,
				LAG(p.NAV_USD) OVER (
					PARTITION BY p.Client_ID, p.Fund_ID
					ORDER BY 
						CAST(SUBSTRING_INDEX(p.`quarter`, ' ', -1) AS UNSIGNED), 
...
FROM nav_change
JOIN clients c ON nav_change.Client_ID = c.Client_ID
JOIN funds f ON nav_change.Fund_ID = f.Fund_ID
WHERE 
...
-- Identify Significant NAV Changes (NAV ±5%) Quarter 1 2025
    ...
    SELECT 
        ...
        LAG(p.NAV_USD) OVER (
            PARTITION BY p.Client_ID, p.Fund_ID
            ORDER BY 
                CAST(SUBSTRING_INDEX(p.`quarter`, ' ', -1) AS UNSIGNED),           -- Year
                CAST(SUBSTRING(SUBSTRING_INDEX(p.`quarter`, ' ', 1), 2, 1) AS UNSIGNED) -- Quarter number from 'Q#'
        ) AS Prev_NAV
    FROM performance p
)
SELECT 
...
WITH irr_trends AS (
    SELECT
        ...
        LAG(p.irr) OVER (
            ...
            ORDER BY 
                CAST(SUBSTRING_INDEX(p.quarter, ' ', -1) AS UNSIGNED), -- Year
                CAST(SUBSTRING(p.quarter, 2, 1) AS UNSIGNED)          -- Quarter
        ) AS previous_irr
    FROM performance p
    JOIN clients c ON p.client_id = c.client_id
    JOIN funds f ON p.fund_id = f.fund_id
...
WHERE quarter = 'Q1 2025'
  AND current_irr < previous_irr   -- only negative growth
